...
contextBridge.exposeInMainWorld('electron', {
    ...
    gitGraph: (rootPath, limit) => ipcRenderer.invoke("git-graph", rootPath, limit),
    gitBranches: (rootPath) => ipcRenderer.invoke("git-branches", rootPath),
    gitDiff: (rootPath, options) => ipcRenderer.invoke("git-diff", rootPath, options),
    gitCommitDiff: (rootPath, options) => ipcRenderer.invoke("git-commit-diff", rootPath, options),
    gitBlame: (rootPath, filePath) => ipcRenderer.invoke("git-blame", rootPath, filePath),
    ...
    gitStageAll: (rootPath) => ipcRenderer.invoke("git-stage-all", rootPath),
    gitUnstageAll: (rootPath) => ipcRenderer.invoke("git-unstage-all", rootPath),
    gitDiscard: (rootPath, filePath, options) => ipcRenderer.invoke("git-discard", rootPath, filePath, options),
    gitRestore: (rootPath, filePath) => ipcRenderer.invoke("git-restore", rootPath, filePath),
    ...
    gitCreateBranch: (rootPath, branch, checkout = true) => ipcRenderer.invoke("git-create-branch", rootPath, branch, checkout),
    gitDeleteBranch: (rootPath, branch) => ipcRenderer.invoke("git-delete-branch", rootPath, branch),
    gitRenameBranch: (rootPath, oldName, newName) => ipcRenderer.invoke("git-rename-branch", rootPath, oldName, newName),
    gitFetch: (rootPath) => ipcRenderer.invoke("git-fetch", rootPath),
    gitPull: (rootPath) => ipcRenderer.invoke("git-pull", rootPath),
    gitPush: (rootPath, currentOnly = true) => ipcRenderer.invoke("git-push", rootPath, currentOnly),
    gitMerge: (rootPath, branch) => ipcRenderer.invoke("git-merge", rootPath, branch),
    gitRebase: (rootPath, branch) => ipcRenderer.invoke("git-rebase", rootPath, branch),
    gitHasChanges: (rootPath) => ipcRenderer.invoke("git-has-changes", rootPath),
